...
beforeAll(() => {
  ...
  tubeNode = Object.values(recursiveTube.nodes).find(n => n.type === 'tube') as TubeNode;
})
...
describe('getNodeEdges', () =>{
  ...
  describe('with prefix', () => {
    test('prefixes sources and targets', () => {
      const edges = getNodeEdges({...baseNode, depends: [{slot: 'b.something'}]}, 'prefix');
    ...
    test('unnests input dependencies with prefix', () => {
      const edges = getNodeEdges({...baseNode, depends: [{slot:'input.something'}]}, 'prefix');
    ...
    test('unnests return targets with prefix', () => {
      const edges = getNodeEdges({id: 'test', type: 'return', depends: [{slot: 'b.something'}]}, 'prefix');
...
describe('getTubeNode', () => {
  ...
  test('removes a return node', () => {
    const return_node = Object.values(tubeNode.params.tube.nodes).find(n=>n.type === 'return');
...
describe(tubeToFlow, () => {
  test('connects nested inputs/returns with outer dependencies', () => {
    ...
    expect(groupNode.data.sourceHandles).toMatchObject([ { id: 'b.value', label: 'value', key: 'b.value' } ] )
  ...
  },
  {
    ...
    const abEdge = recursiveEdges.find(e=>e.id==='a.index-b.child_multiply_inner')

    expect(abEdge).toMatchObject({source: 'a', target: 'b', sourceHandle: 'a.index', targetHandle: 'b.child_multiply_inner'})
