...
- [`#252`](https://github.com/miniscope/noob/pull/252) - {meth}`.YAMLMixin.from_any` now allows a specification to be passed as a dict.
- [`#252`](https://github.com/miniscope/noob/pull/252) - For documentation purposes, a generic `__getattr__` was added to `noob.testing`,
  so that when referring to "some node" an actual passthrough node can be gotten.
- [`#252`](https://github.com/miniscope/noob/pull/252) - Add a pyodide runner so that noob tubes can run directly in the docs!
...
- [`#256`](https://github.com/miniscope/noob/pull/256) - Generators and other nodes that have some limited lifespan may now emit a `MetaSignal.Exhausted`
...
- [`#261`](https://github.com/miniscope/noob/pull/261) - added `@init_method` and `@deinit_method` decorators

**Changed**

- [`#251`](https://github.com/miniscope/noob/pull/251) - Use pdm's experimental workspace support
- [`#252`](https://github.com/miniscope/noob/pull/252) - `TubeSpecification`s yielded in the GUI viewer are now wrapped in `SpecMsg`s,
  so that all point-to-point communication within noob stays within a single message format.
- [`#252`](https://github.com/miniscope/noob/pull/252) - {class}`.Gather` now uses a {class}`threading.Lock` rather than `multiprocessing` -
  multiprocessing was never necessary and isn't compatible with pyodide.
- [`#252`](https://github.com/miniscope/noob/pull/252) - {class}`.ProcessMsg` can have `epoch` be `None`, when e.g. being controlled by some proxy
  that allows the runner (specifically, its scheduler) to decide the epoch.
- [`#256`](https://github.com/miniscope/noob/pull/256) - Runners now no longer automatically `deinit` at the end of a `run` or `iter` call:
...
- [`#251`](https://github.com/miniscope/noob/pull/251) - A race condition could cause ZMQ runners to hang due to slow subscribers in ZMQ PUB/SUB sockets,
...
- [`#252`](https://github.com/miniscope/noob/pull/252) - {class}`.NodeSpecification` ignores `nodeinfo` when validating,
  so specs can again be round-tripped.
- [`#252`](https://github.com/miniscope/noob/pull/252) - {class}`.TubeSpecification`s are serialized by alias so that e.g. `type_` comes out as `type`
- [`#256`](https://github.com/miniscope/noob/pull/256) - `deinit` on wrapped generator nodes actually deinitializes them, and `init` recreates them.
- [`#260`](https://github.com/miniscope/noob/pull/260) - Optional propagation now honors slots.
...
- [`#260`](https://github.com/miniscope/noob/pull/260) - The `EventStore` no longer returns `NoEvent`s from `collect()`.
  NoEvents are never intended to be given to a node as arguments, and `collect` is only used to make node args/kwargs.
- [`#262`](https://github.com/miniscope/noob/pull/262) - When a return node excludes keys that were not emitted when used in a Tube Node,
  fill in `NoEvent`s.
...
You can see the {doc}`noob-core </api/noob-core/index>` documentation for more details on the implementation.
